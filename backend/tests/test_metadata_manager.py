...
class TestGenerateFallbackVariations:
    ...
    def test_generates_correct_count(self):
        ...
        from server import generate_fallback_variations

        variations = generate_fallback_variations("A cat", 3)
        assert len(variations) == 3

    def test_variations_have_required_fields(self):
        """Each variation has id, type, description, mood."""
        from server import generate_fallback_variations

        variations = generate_fallback_variations("A dog", 2)
        for var in variations:
            assert "id" in var
            assert "type" in var
            assert var["type"] == "fallback"
            assert "description" in var
            assert "mood" in var

    def test_first_variation_uses_original_prompt(self):
        """First variation uses original prompt without suffix."""
        from server import generate_fallback_variations

        variations = generate_fallback_variations("A bird", 3)
        assert variations[0]["description"] == "A bird"

    def test_subsequent_variations_use_suffixes(self):
        """Subsequent variations add suffixes."""
        from server import generate_fallback_variations

        variations = generate_fallback_variations("A fish", 3)
        # First has no suffix, subsequent do
        assert variations[0]["description"] == "A fish"
        assert variations[1]["description"] != "A fish"
        assert variations[2]["description"] != "A fish"

    def test_fills_existing_variations(self):
        """Can fill existing variations list to reach target count."""
        from server import generate_fallback_variations

        existing = [{"id": "1", "type": "parsed", "description": "Existing", "mood": "happy"}]
        variations = generate_fallback_variations("Base prompt", 3, existing_variations=existing)

        assert len(variations) == 3
        assert variations[0]["type"] == "parsed"  # Original kept
        assert variations[1]["type"] == "fallback"
        assert variations[2]["type"] == "fallback"
